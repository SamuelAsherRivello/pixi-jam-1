...
class Animations {
  ...
  spinAndShrink(target: Container, delayMs: number, durationMs: number, onStart: () => void, onComplete: () => void): Action {
    let action = Actions.sequence(
      ...
      Actions.parallel(
        Actions.rotateTo(target, 1, durationMs, Interpolations.smooth2),

        Actions.scaleTo(target, 0, 0, durationMs, Interpolations.smooth2)
      ),
      Actions.runFunc(() => {
        onComplete();
      })
  ...
  riseAndGrow(target: Container, delayMs: number, durationMs: number, onStart: () => void, onComplete: () => void): Action {
    let action = Actions.sequence(
      ...
      Actions.parallel(
        Actions.moveTo(target, target.x, target.y - target.height, durationMs, Interpolations.smooth2),

        Actions.scaleTo(target, 2, 2, durationMs, Interpolations.smooth2)
      ),
      Actions.runFunc(() => {
        onComplete();
      })
